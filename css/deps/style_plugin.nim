...
## calls that register the component's styles into the aoughwl substrate.
##
## Authored within aoughwl via `aoughwl/plugin`. Rather than a hand-written
## in→out transform, it *searches for* every declaration in the block and
## *applies* a rewrite to each — `searchForAll(decl).apply(toStyleOne)`.
##
## Input shape: (stmts style "name" (stmts (call <prop> (stmts <value>)) ...))

import aoughwl/plugin
...
proc transform(input: Node): Node =
  ...
  # apply the rewrite that turns it into a `styleOne(name, prop, value)` call.
  body.rewrite(
